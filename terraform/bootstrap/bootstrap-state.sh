...
# Format the incoming data from the config file (strip spaces and quotes)
...
AWS_REGION=$(grep '^region[[:space:]]*=' "$CONFIG_PATH" | cut -d'"' -f2 | tr -d '[:space:]')
...
check_stack() {
    aws cloudformation describe-stacks --region "$AWS_REGION" --stack-name "$STACK_NAME" 2>/dev/null
    return $?
}
...
deploy_stack() {
    ...
    aws cloudformation wait stack-${operation}-complete --region "$AWS_REGION" --stack-name "$STACK_NAME"
}
